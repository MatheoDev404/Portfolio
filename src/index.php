<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.1.0/css/all.css" integrity="sha384-lKuwvrZot6UHsBSfcMvOkWwlCMgc0TaWr+30HWe3a4ltaBwTZhyTEggF5tJv8tbt" crossorigin="anonymous"> 
    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" href="assets/css/projets.css">
    <link rel="stylesheet" href="assets/css/default.css" />
    <link rel="stylesheet" href="assets/css/navigation.css" />
    <title>Mathéo STUNAULT - développeur/intégrateur WEB</title>
</head>
<body>
    <main>
        <section id="home">
            <h1>Développeur &#38; Intégrateur Web<span> Mathéo Stunault</span></h1>
            <a href="#projects"> 
                <i class="fas fa-arrow-circle-down"></i> 
            </a>
        </section>
        <nav id="menu" class="nav">					
            <ul>
                <li>
                    <a href="#projects">
                        <span class="icon">
                            <i aria-hidden="true" class="far fa-folder-open"></i>
                        </span>
                        <span class="categorie">Projets</span>
                    </a>
                </li><li>
                    <a href="#about">
                        <span class="icon"> 
                            <i aria-hidden="true" class="fas fa-user-circle"></i>
                        </span>
                        <span class="categorie">A propos</span>
                    </a>
                </li><li>
                    <a href="#contact">
                        <span class="icon">
                            <i aria-hidden="true" class="icon-contact"></i>
                        </span>
                        <span class="categorie">Contact</span>
                    </a>
                </li>
            </ul>
        </nav>
        <section id="projects">
            <h2 class="section_title">Projets</h2>
            <div id="container_projects">
                <div class="container-img">
                    <div class="hovereffect">
                        <img  src="assets/img/site-chat.png" alt="miniature d'un projet, site d'adoption de chats.">
                        <div class="overlay">
                            <h2 class="black">OhMyCats !</h2>
                            <p class="black">Mise en forme d'un site web responsive d'adoption de chats, en utilisant: HTML/CSS, Less, Bootstrap, JavaScript.</p>
                            <a class="white" href="#">Voir le projet</a>
                        </div>
                    </div>
                </div>
                <div class="container-img">
                    <div class="hovereffect">
                        <img  src="assets/img/site-licorne.png" alt="miniature d'un projet, site d'adoption de chats.">
                        <div class="overlay">
                            <h2 class="black">MyUnicorne</h2>
                            <p class="black">Mise en forme d'un site web responsive d'un élevage de licornes, en utilisant: HTML/CSS.</p>
                            <a class="white" href="#">Voir le projet</a>
                        </div>
                    </div>
                </div>
                
                <div class="container-img">
                    <div class="hovereffect">
                        <img class="img-responsive" src="http://placehold.it/350x250" alt="">
                        <div class="overlay">
                            <h2>titre du projet</h2>
                            <p>description du projet, technologie etc Lorem ipsum dolor sit amet consectetur adipisicing elit. Libero facilis distinctio, dolores deserunt dolore minima quibusdam nihil laudantium necessitatibus consequatur.</p>
                            <a href="#">Voir le projet</a>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="about">
            <h2 class="section_title"><span class="uppercase">à</span> propos</h2>
            <div id="presentation">
                <h2>Présentation</h2>
                <p>Je suis un jeune developpeur web, attiré par le web et tout ce qui s'y passe. 
                    <span class="display-block"></span>
                    Après mon BAC, 
                    j'ai donc entamé une formation à l'école multimédia pendant 2 années, 
                    puis par manque de moyens, 
                    j'ai donc continué en autodidacte tout en travaillant en parallèle, 
                    notamment chef d'une équipe de jardiniers. 
                    <span class="display-block"></span>
                    Aujourd'hui, 
                    je suis une formation en développement web et mobile au sein de Webforce3, 
                    labellisée Grande Ecole du Numérique (GNE), 
                    afin de confirmer mes acquis de les solidifier.</p>
            </div>  
            <section>
                <div class="competences">
                    <h3 class="about_title">Mes compétences</h3>
                    <ul id="liste-competences">
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="html-css-bar"><h5>HTML5/CSS3</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="js-bar"><h5>JavaScript</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="jquery-bar"><h5>jQuery</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="angular-bar"><h5>Angular</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="php-bar"><h5>PHP</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="mysql-bar"><h5>MySQL</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="symfony-bar"><h5>Symfony</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="laravel-bar"><h5>Laravel</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="ionic-bar"><h5>Ionic</h5></div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div>
                                <div class="jauge">
                                    <div class="sass-bar"><h5>Sass/Less</h5></div>
                                </div>
                            </div>
                        </li>
                       
                    </ul>
                </div>
                <div class="parcours">
                    <h3 class="about_title">Mon parcours</h3>
                   
                    </ul>
                    <div class="cv_button">
                        <a href="#"> Télécharger mon CV</a>
                    </div>
                </div>
            </section>
        </section>
        <section id="contact">
            <h2 class="section_title">Contact</h2> 
            <div class="left">
                <h3 class="contact_title">infos contact</h3>
                <ul>
                    <li>
                        <p>adresse</p>
                    </li>
                    <li>
                        <p>telephone</p>
                    </li>
                    <li>
                        <p>email</p>
                    </li>
                </ul>
            </div>
            <div class="right">
                <h3 class="contact_title">formulaire de contact</h3>
                <form action="#" method="post" enctype="multipart/form-data">

                    <label class="hidden" for="nom" class="hidden">Nom</label>
                    <input type="text" id="nom" name="nom" placeholder="Nom" required>

                    <label class="hidden" for="prenom" class="hidden">Prénom</label>
                    <input type="text" id="prenom" name="prenom" placeholder="Prénom" required>

                    <label class="hidden" for="email">Email</label>
                    <input type="email" id="email" name="email" placeholder="Email">

                    <label class="hidden" for="objet" class="hidden">Objet</label>
                    <input type="text" id="objet" name="objet" placeholder="Objet du message" required>
                   
                    <label class="hidden" for="message">message</label>
                    <textarea name="message" id="message" placeholder="Message" cols="25" rows="5"></textarea>
                    
                    <div id="envoye">
                        <input type="submit" name="envoyer" value="Envoyer" />
                    </div>
                    
                </form>            
            </div>       
        </section>
    </main>
    <footer id="main_footer">
        <ul id="networks">
            <li><a href="https://github.com/MatheoDev404"><i class="fab fa-github"></i></a></li>
            <li><a href="https://www.linkedin.com/in/math%C3%A9o-stunault-ab1533164/"><i class="fab fa-linkedin"></i></a></li>
            <li><a href="https://twitter.com/MDev404"><i class="fab fa-twitter-square"></i></a></li>
        </ul>
        <div>
            <p>Copyright &copy; 2018, Mathéo Stunault</p>
        </div>
    </footer>

    <!-- //Modernizr -->
	<script src="assets/js/modernizr.custom.js"></script>

    <!-- //jQuery -->
    <script src="https://code.jquery.com/jquery-3.1.0.js"></script>

    <!-- //Js -->
    <script src="assets/js/main.js"></script>
</body>
</html>